...
if ( nargin == 2 )
    leaf = leaves(f);
    for k = 1:length(leaf)
        ff = treefun2.coeffs2vals(leaf(k).coeffs);
        ff(abs(ff) < 1e-20) = 1e-20;
        f.boxes(leaf(k).id).coeffs = treefun2.vals2coeffs(op(ff));
    end
elseif ( isa(f, 'treefun2') && isa(g, 'treefun2') )
    ...
    for k = 1:length(leaff)
        ff = treefun2.coeffs2vals(leaf_f(k).coeffs);
        gg = treefun2.coeffs2vals(leaf_g(k).coeffs);
        f.boxes(leaf_f(k).id).coeffs = treefun2.vals2coeffs(op(ff,gg));
    end
elseif ( isa(f, 'treefun2') )
    ...
    for k = 1:length(leaf)
        ff = treefun2.coeffs2vals(leaf(k).coeffs);
        f.boxes(leaf(k).id).coeffs = treefun2.vals2coeffs(op(ff,g));
    end
elseif ( isa(g, 'treefun2') )
    % F is not a TREEFUN2:
    leaf = leaves(g);
    for k = 1:length(leaf)
        gg = treefun2.coeffs2vals(leaf(k).coeffs);
        g.boxes(leaf(k).id).coeffs = treefun2.vals2coeffs(op(f,gg));
    end
    f = g;
end
